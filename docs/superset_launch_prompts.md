...
### AGENT 4 — TRAINING
Directory: `train/`. Follow the `TRAINING` section. `filter.py` (config-driven, emits chat-format JSONL + manifest; drops counterfactual pairs whose two decisions are identical), `launch.py` (`--backend {fireworks,sagemaker,ec2}`, default `fireworks`: uploads the filtered JSONL as a Fireworks **dataset** and creates SFT **LoRA** jobs through the Fireworks REST API / `firectl` CLI, one job per sweep variant, over `accounts/fireworks/models/llama-v3p1-8b-instruct` and the Qwen2.5-7B-Instruct equivalent — **verify both exact model IDs against the Fireworks model library at run time and use what it returns**, do not hardcode a guess; `sagemaker` and `ec2` (PEFT/Unsloth) stay available for when GPU quota lands; rank/epochs from config; provider dataset/job/model ids and checkpoints to S3 with all version tags), `continue.py` (Fireworks **continue-from-LoRA** off an existing tuned adapter), `dpo.py` (Fireworks **DPO** job type fed a preference-pair JSONL: pairs from the lake on a chosen judge dimension, one epoch, logs a patch note), `serve.py` (creates **one** Fireworks on-demand deployment and loads **every** sweep adapter onto it as **multi-LoRA**; exposes `engine.agent_api` as a thin client that selects the adapter by name per request; `--down` deletes the deployment. For `sagemaker`/`ec2` it falls back to vLLM on EC2), `gates.py` (held-out persona coherence, counterfactual sensitivity, schema validity), `devset.py` (runs `specs/devset/` scenarios against a served checkpoint and writes a metrics table per `contracts/targets.md`). Start with a smoke test on 200 examples to validate the whole path end to end before anything else. Do not run real replays or the held-out year. **No local GPU.** The EC2 G/VT on-demand quota (`L-DB2E81BA`) is currently **0** with an increase request pending (`312f3b0f78754d25920d9b0f6482d2feWs3fPUjY`, `CASE_OPENED`), and every SageMaker g5 training quota is also 0. Do not assume a GPU instance exists; the `fireworks` backend is the default for that reason. **Deployment costs money.** The Fireworks on-demand deployment is billed **per GPU-hour** — bring it up only for dev-set eval, round-2 generation, and final validation, tear it down (`serve.py --down`) between each, and log every up/down time to `docs/VERSIONS.md`.
...
### AGENT 8 — INFRA
Directory: `infra/`. Create S3 buckets/prefixes per `contracts/s3_layout.md` (Terraform or a boto3 script; idempotent). Provision a GPU instance (g5.12xlarge or p4d) **only if** `aws service-quotas get-service-quota --service-code ec2 --quota-code L-DB2E81BA` shows a value **> 0**; otherwise skip provisioning entirely and report the pending request ID (`312f3b0f78754d25920d9b0f6482d2feWs3fPUjY`) in `REPORT.md`. If provisioned, use an AMI that has CUDA, PyTorch, PEFT, Unsloth, vLLM installed; write `infra/gpu_setup.sh`. Provision a small EC2 or use local for the engine. Write `infra/serve_vllm.sh <checkpoint_s3_uri>`. IAM role with S3 read/write scoped to the bucket. Cost guardrails: tag everything `project=svalbard`, write `infra/teardown.sh`. Report instance IDs and endpoints in `REPORT.md`.
