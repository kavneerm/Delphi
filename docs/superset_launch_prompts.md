...
### AGENT 4 — TRAINING
Directory: `train/`. Follow the `TRAINING` section. `filter.py` (config-driven, emits chat-format JSONL + manifest; drops counterfactual pairs whose two decisions are identical), `launch.py` (`--backend {together,sagemaker,ec2}`, default `together`: hosted LoRA fine-tuning on Together — or Fireworks — over JSONL uploaded to the provider, bases Llama-3.1-8B and Qwen2.5-7B, tuned model served from the provider's endpoint; `sagemaker` and `ec2` (PEFT/Unsloth) stay available for when GPU quota lands; rank/epochs from config; checkpoints and provider job ids to S3 with all version tags), `continue.py`, `dpo.py` (pairs from the lake on a chosen judge dimension, one epoch, logs a patch note), `serve.py` (thin client to whichever backend produced the checkpoint — the provider endpoint for `together`, vLLM on EC2 for `sagemaker`/`ec2` — exposing `engine.agent_api`), `gates.py` (held-out persona coherence, counterfactual sensitivity, schema validity), `devset.py` (runs `specs/devset/` scenarios against a served checkpoint and writes a metrics table per `contracts/targets.md`). Start with a smoke test on 200 examples to validate the whole path end to end before anything else. Do not run real replays or the held-out year. **No local GPU.** The EC2 G/VT on-demand quota (`L-DB2E81BA`) is currently **0** with an increase request pending (`312f3b0f78754d25920d9b0f6482d2feWs3fPUjY`, `CASE_OPENED`). Do not assume a GPU instance exists; the `together` backend is the default for that reason.
